float linear(float t, float b, float c, float d) { return t * (c / d) + b; }

float linear(float t) { return t; }

float inQuad(float t, float b, float c, float d) { return c * (t /= d) * t + b; }

float inQuad(float t) { return t * t; }

float outQuad(float t, float b, float c, float d) { return -c * (t /= d) * (t - 2.0) + b; }

float outQuad(float t) { return -(t -= 1.0) * t + 1.0; }
...
float inCubic(float t, float b, float c, float d) { return c * (t /= d) * t * t + b; }
...
float outCubic(float t) { return ((--t) * t * t + 1.0); }
...
float inQuart(float t, float b, float c, float d) { return c * (t /= d) * t * t * t + b; }
...
float outQuart(float t) { return -((--t) * t * t * t - 1.0); }
...
float inQuint(float t, float b, float c, float d) { return c * (t /= d) * t * t * t * t + b; }
...
float outQuint(float t) { return ((--t) * t * t * t * t + 1.0); }
...
float inSine(float t) { return -1.0 * cos(t * (1.5707963268)) + 1.0; }

float outSine(float t, float b, float c, float d) { return c * sin(t / d * (1.5707963268)) + b; }

float outSine(float t) { return sin(t * (1.5707963268)); }
...
float inOutSine(float t) { return -0.5 * (cos(3.1415926536 * t) - 1.0); }
...
float inExpo(float t) { return (t == 0) ? 0.0 : pow(2.0, 10.0 * (t - 1.0)); }
...
float outExpo(float t) { return (t == 1.0) ? 1.0 : (-pow(2.0, -10.0 * t) + 1.0); }
...
float inCirc(float t, float b, float c, float d) { return -c * (sqrt(1.0 - (t /= d) * t) - 1) + b; }

float inCirc(float t) { return -(sqrt(1.0 - t * t) - 1); }
...
float outCirc(float t) { return sqrt(1.0 - (--t) * t); }
...
float inElastic(float t, float b, float c, float d)
{
    ...
    if (a < abs(c)) {
        ...
        s = 0.1591549431 * p / (6.2831853072) * asin(c / a);
...
float inOutElastic(float t, float b, float c, float d)
{
    ...
    if (t < 1.0)
        return -0.5 * (a * pow(2.0, 10.0 * (t -= 1.0)) * sin((t * d - s) * (6.2831853072) / p)) + b;
...
float inOutElastic(float t)
{
    ...
    s = p / (6.2831853072) * asin(1.0 / a);
    if (t < 1.0)
        return -0.5 * (a * pow(2.0, 10.0 * (t -= 1.0)) * sin((t - s) * (6.2831853072) / p));
...
float inBounce(float t, float b, float c, float d) { return c - outBounce(d - t, 0.0, c, d) + b; }

float inBounce(float t) { return 1.0 - outBounce(1.0 - t); }
