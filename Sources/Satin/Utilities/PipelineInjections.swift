...
func injectTexturesArgs(source: inout String, maps: Set<PBRTexture>) {
    var injection = ""
    for map in maps {
...
func injectShadowSampler(source: inout String, receiveShadow: Bool) {
    let injection = "constexpr sampler shadowSampler(min_filter::nearest, mag_filter::nearest);\n"
    source = source.replacingOccurrences(of: "// inject shadow sampler\n", with: receiveShadow ? injection : "")
}
...
func injectShadowCoords(source: inout String, receiveShadow: Bool) {
    let injection = "float4 shadowCoord0;\n"
...
func injectShadowFragmentArgs(source: inout String, receiveShadow: Bool) {
    let injection = "depth2d<float> shadowDepthTex [[texture(FragmentTextureCustom0)]],\n"
    source = source.replacingOccurrences(of: "// inject shadow fragment args\n", with: receiveShadow ? injection : "")
}

func injectShadowVertexCalc(source: inout String, receiveShadow: Bool) {
    let injection = "out.shadowCoord0 = shadowMatrices[0] * in.position;\n"
    source = source.replacingOccurrences(of: "// inject shadow vertex calc\n", with: receiveShadow ? injection : "")
}

func injectShadowFragmentCalc(source: inout String, receiveShadow: Bool) {
    let injection =
    """
    float shadow = 1.0;

    float4 shadowCoord = in.shadowCoord0;
    ...
    if(frustumTest) {
        shadow = shadowCoord.z > shadowDepthTex.sample(shadowSampler, shadowCoord.xy) ? 1.0 : 0.25
        ;
    }
