...
float calculateShadow( float4 shadowPosition, float4 position, depth2d<float> shadowTexture )
{
	...
	const float2 shadowTextureSize = float2( shadowTexture.get_width(), shadowTexture.get_height() ) * 1.0;
	float shadow_sample = 0.0;
	const int total = 4;
	for( int i = 0; i < total; i++ ) {
		int index = int( 16.0 * random( floor( position.xyz * 1000.0 ), i ) ) % 16;
		index *= 2;
		const float2 poisson = float2( poissonDisk16[index], poissonDisk16[index + 1] ) / shadowTextureSize;
