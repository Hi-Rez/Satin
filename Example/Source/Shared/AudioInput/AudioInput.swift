...
class AudioInput: NSObject, AVCaptureAudioDataOutputSampleBufferDelegate {
    ...
    public lazy var input: StringParameter = {
        let param = StringParameter("Input", "", inputs, .dropdown) { [unowned self] value in
            self.setupCapture()
        }
        return param
    }()
    ...
    public lazy var rmsTextureType: StringParameter = {
        StringParameter("RMS Texture", "rms-smoothed", rmsTextureTypes, .dropdown)
    }()
    
    public var windowTypes: [String] = ["Hanning-N", "Hanning-D", "Hamming", "Blackman", "None"]
    public lazy var window: StringParameter = {
        let param = StringParameter("Spectrum Filter", "Blackman", windowTypes, .dropdown) { [unowned self] value in
            self.setWindowType()
        }
        return param
    }()
    
    public lazy var fftSmoothing: FloatParameter = {
        let param = FloatParameter("Spectrum Smoothing", 0.975, .slider) { [unowned self] value in
            self.setSmoothing()
        }
        return param
    }()
