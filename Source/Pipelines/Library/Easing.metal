...
float outElastic(float t, float b, float c, float d) {
    ...
    return a * pow(2.0, -10.0 * t) * sin((t * d - s) * (6.2831853072) / p) + c +
           b;
}
...
float inOutBack(float t, float b, float c, float d) {
    ...
    return c / 2.0 * ((t -= 2.0) * t * (((s *= (1.525)) + 1.0) * t + s) + 2.0) +
           b;
}
